...
export default function RegisterPage() {
  ...
  return (
    <div className="mx-auto max-w-xl rounded-[2.1rem] border border-white/10 bg-[radial-gradient(circle_at_top,_rgba(245,200,74,0.12),_transparent_22%),radial-gradient(circle_at_top_left,_rgba(56,189,248,0.14),_transparent_24%),linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.38)]">
      ...
      <h1 className="mt-4 text-4xl font-semibold text-white">Create your private Vypexrock workspace</h1>
      <p className="mt-3 text-sm leading-7 text-white/60">Register first to use the dashboard, live prices, Vypexrock AI, community features, and your personal trading tools.</p>
      <form onSubmit={handleSubmit} className="mt-8 space-y-4">
        <Field label="Full name" type="text" value={fullName} onChange={setFullName} />
        <Field label="Email" type="email" value={email} onChange={setEmail} />
        <Field label="Password" type="password" value={password} onChange={setPassword} />
        {error ? <p className="text-sm text-rose-400">{error}</p> : null}
        <button className="w-full rounded-full bg-gradient-to-r from-cyan-400 via-violet-500 to-fuchsia-500 px-4 py-3 font-semibold text-slate-950">
